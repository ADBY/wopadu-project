...
//exit;
		...
		{		
			if($model->load(Yii::$app->request->post()))
			{
				...
				if($model->option_name != "") {				
					foreach($language as $lang) {
						$name_in_lang = "option_name_".$lang['language_lc'];
						$translated_array = Yii::$app->mycomponent->bingTraslateAPI($model->option_name, "en", $lang['language_code']);
				...
				if($gen_error == "" && $model->save()) {
					...
					Yii::$app->mycomponent->translateItemNotif(['type' => 5, 'item_id' => $option_main_id]);
					...
					for($i = 0; $i< count($options_data['option_name']); $i++)
					{
						if($options_data['option_name'][$i] != "") {
							...
							if($model_sub->sub_name != "") {				
								foreach($language as $lang) {
									$name_in_lang = "sub_name_".$lang['language_lc'];
									$translated_array = Yii::$app->mycomponent->bingTraslateAPI($model_sub->sub_name, "en", $lang['language_code']);
									if($translated_array['response'] == "OK")
									{
										$model_sub->$name_in_lang = $translated_array['translation'];
									}
									else
									{
										$model_sub->$name_in_lang = "";
									}
								}
							}
							...
							$model_sub->save();
							Yii::$app->mycomponent->translateItemNotif(['type' => 6, 'item_id' => $model_sub->id]);
						}
					}
    ...
    public function actionUpdate($id)
    {
		...
		foreach($language as $lang)
		{
			$item_op_str .= "IOS.sub_name_".$lang['language_lc'].", ";
		}
		
		$item_op_str = rtrim($item_op_str, ", ");
		
		$item_options = Yii::$app->db->createCommand("SELECT IOM.id, IOM.option_name, IOS.sub_name, IOS.sub_amount, ".$item_op_str." FROM item_option_main as IOM INNER JOIN item_option_sub as IOS ON IOS.option_id = IOM.id WHERE IOM.id = ".$id)->queryAll();
		...
		foreach($item_options as $i)
		{
			...
			foreach($language as $lang)
			{
				$options_data['sub_name_'.$lang['language_lc']][] = $i['sub_name_'.$lang['language_lc']];
			}			
		}
        ...
        if($model->load(Yii::$app->request->post()))
		{
			...
			foreach($language as $lang)
			{
				$options_data['sub_name_'.$lang['language_lc']] = $_POST['sub_name_'.$lang['language_lc']];
			}
			/************* temp for translate ********/
			/*if($model->option_name != "") {				
				foreach($language as $lang) {
			...
			if($model->save())
			{
				...
				for($i = 0; $i< count($options_data['option_name']); $i++)
				{
					if($options_data['option_name'][$i] != "") {
						...
						foreach($language as $lang)
						{
							if(!isset($options_data['sub_name_'.$lang['language_lc']][$i]) || is_array($options_data['sub_name_'.$lang['language_lc']][$i]))
							{
								$name_in_lang = "sub_name_".$lang['language_lc'];
								$translated_array = Yii::$app->mycomponent->bingTraslateAPI($model_sub->sub_name, "en", $lang['language_code']);
								if($translated_array['response'] == "OK")
							...
							else if($options_data['sub_name_'.$lang['language_lc']][$i] != "" && !is_array($options_data['sub_name_'.$lang['language_lc']][$i]))
							{
								$name_in_lang = "sub_name_".$lang['language_lc'];
								$model_sub->$name_in_lang = $options_data['sub_name_'.$lang['language_lc']][$i];
							}
						}						
						$model_sub->save();
						Yii::$app->mycomponent->translateItemNotif(['type' => 6, 'item_id' => $model_sub->id]);
