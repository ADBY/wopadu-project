...
class ItemsController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if($model->load(Yii::$app->request->post()))
		{
			...
			$fileName 		= $_FILES["Items"]["name"]["images"]; // The file name
			$fileTmpLoc 	= $_FILES["Items"]["tmp_name"]["images"]; // File in the PHP tmp folder
			$fileType 		= $_FILES["Items"]["type"]["images"]; // The type of file it is
			$fileSize 		= $_FILES["Items"]["size"]["images"]; // File size in bytes
			$fileErrorMsg 	= $_FILES["Items"]["error"]["images"]; // 0 for false... and 1 for true
			...
			if($gen_error == "" && $model->save())
			{
				...
				Yii::$app->mycomponent->translateItemNotif(['type' => 3, 'item_id' => $model->id]);
