...
class StoresController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if($model->load(Yii::$app->request->post()))
		{
			...
			$fileName 		= $_FILES["Stores"]["name"]["image"]; // The file name
			$fileTmpLoc 	= $_FILES["Stores"]["tmp_name"]["image"]; // File in the PHP tmp folder
			$fileType 		= $_FILES["Stores"]["type"]["image"]; // The type of file it is
			$fileSize 		= $_FILES["Stores"]["size"]["image"]; // File size in bytes
			$fileErrorMsg 	= $_FILES["Stores"]["error"]["image"]; // 0 for false... and 1 for true
			...
			if($gen_error == "" && $model->save())
			{
				...
				Yii::$app->mycomponent->translateItemNotif(['type' => 1, 'item_id' => $model->id]);
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post()))
		{
			...
			if($model->stripe_publishable_key == "" || $model->stripe_user_id == "" || $model->refresh_token == "" || $model->access_token == "")
			{
				$model->stripe_details_flag = 0;
				$model->status = 0;
			}
			else
			{
				$model->stripe_details_flag = 1;
			}
