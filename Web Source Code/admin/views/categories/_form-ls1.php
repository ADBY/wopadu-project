...
<?php if(!$model->isNewRecord) { ?>
				...
				<?php
                   /* if(!$model->isNewRecord && $allowed_languages != "") { 
                    
                    $language = Yii::$app->db->createCommand("SELECT language_name, language_lc FROM languages WHERE id IN (".$allowed_languages.")")->queryAll();
                    
                    foreach($language as $lang) {
                        echo $form->field($model, 'item_name_'.$lang['language_lc'])->textInput(['maxlength' => true, 'placeholder' => 'Please enter product name in '.$lang['language_name']]);
						echo $form->field($model, 'item_description_'.$lang['language_lc'])->textArea(['rows' => 6, 'placeholder' => 'Please enter product description in '.$lang['language_name']]);
                    }
