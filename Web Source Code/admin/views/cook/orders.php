    ...
    <div class="col-md-6">
        ...
        <section class="panel panel-<?php if($order['status'] == 2) { echo "orange"; } else if($order['status'] == 1) { echo "danger"; } else if($order['status'] == 5) { echo "grey"; } else if($order['status'] == 3) { echo "success"; } ?>" id="order_panel_<?= $order['id'] ?>">
            <header class="panel-heading">
                ...
                <span class="t-left" style="width:50%; text-transform:capitalize"><?php if($order['table_location'] == "Takeaway") { echo $order['table_location']; } else { echo $order['table_location']; } ?></span>
                ...
                <span class="t-right" style="width:48%; text-transform:capitalize">
				<?php 
				if($order['status'] == 5) {
                    ...
                    <div>
                    	<div class="col-md-6 pad-left-0">
                        	<p>
                                ...
                                <br />
                                Customer - <?php echo $order['username']; ?>
                                <br />
								<?= $item['item_name'] ?> 
                                <span style="margin-left:10px;">x</span> <?= $item['quantity'] ?><br />
                            </p>
                        </div>
                        ...
                        <div class="clearfix"></div>
                        <?php if($item['item_variety_id'] != NULL) { 
						//echo '<span style="margin-left:15px">'.$item['item_variety_name'].'</span></br>'; 
						echo '<div style="margin-left:15px"><p><span class="col-md-3" style="padding-left:0">Size </span><span class="col-md-9"> - '.$item['item_variety_name'].'</span></p></div>';
						} ?>
                        <?php foreach($item['options'] as $option) {
                        	echo '<div style="margin-left:15px"><p><span class="col-md-3" style="padding-left:0">'.$option['option_name'].'</span><span class="col-md-9"> - '.$option['sub_name'].'</span></p></div>';
                        } ?>
                        <?php if($item['add_note'] != "") { ?><p style="margin-left:15px">Note: <?= $item['add_note'] ?></p><?php } ?>
                    </div>
                    <?php } ?>
                    <?php if($order['add_note'] != "") { ?><p>Note: <?= $order['add_note'] ?></p><?php } ?>
                    <?php if($order['allergies'] != "") { ?><p style="">Allergies: <?= $order['allergies'] ?></p><?php } ?>
                    <?php } else { $x = 1;/**** For cancelled Orders **/?>
                    <div class="col-md-12" style="font-weight: bold;color: #ff0000;margin-bottom: 15px;">
                    <p>Order Cancelled</p>
                    </div>
                    <?php foreach($order['ordered_items'] as $item) { ?>
                    <div class="col-md-4">
                        <p class="col-md-12 pad-left-0">
                        	<?= $item['item_name'] ?>
                            <span style="margin-left:10px;">x</span> <?= $item['quantity'] ?>
                        </p>
                        <?php foreach($item['options'] as $option) {
                        	echo '<p style=""> '.$option.'</p>';
                        } ?>
                        <?php if($item['add_note'] != "") { ?><p style="">Note: <?= $item['add_note'] ?></p><?php } ?>
                    </div>
