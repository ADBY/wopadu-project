        ...
        <section class="panel panel-primary">
            ...
            <div class="panel-body">
				<form class="form-horizontal" role="form" method="post" id="gen-report-form" action="<?= Yii::$app->urlManager->createUrl('reports/generate-report') ?>" onSubmit="return genReport()">
                    ...
                    <?php if($session['login_role'] == 1) { // Login as Super Admin ?>
                    ...
                    <?php } else if($session['login_role'] == 2) { // Login as Account ?>
                    <input type="hidden" name="account" value="<?php echo $session['account_id']; ?>" >
                    <div class="form-group">
                        <label for="inputPassword1" class="col-md-2 col-sm-2 control-label">Stores</label>
                        <div class="col-md-8">
                            <select class="form-control m-bot15" id="store" name="store">
                                ...
                                <?php
                                $store_list = Yii::$app->db->createCommand("SELECT id, store_name FROM stores WHERE account_id =".$session['account_id'])->queryAll();
								if($store_list)
								{
                                	echo '<option value="0"';
									
									if(isset($posted_data['store']) && $posted_data['store'] == 0)
									{
									...
									foreach($store_list as $ss)
									{
										echo '<option value="'.$ss['id'].'"';
										
										if(isset($posted_data['store']) && $posted_data['store'] == $ss['id'])
										{
											echo ' selected="selected"';
										}
										
										echo '>'.$ss['store_name'].'</option>';
									}
								}
								else
								{
