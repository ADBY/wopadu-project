...
function genReport()
{
	...
	if(account == "") {
		...
		$("#msg-lable").html('');
		return true;
		//alert($("#gen-report-form").length);
		//$("#gen-report-form").submit();
		/*
		var _csrf = yii.getCsrfToken();
		$.ajax({
			url: '<?= Yii::$app->urlManager->createUrl('site-info/gen-report') ?>',
			method: 'POST',
			data: 'account='+account+'&store='+store+'&oType='+oType+'&start_date='+start_date+'&end_date='+end_date+'&_csrf='+_csrf,
			cache: false,
			success: function(data){
