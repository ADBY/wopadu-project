...
*	Register User
*	URL:
http://localhost:8080/wopado/ws/register.php?ws=1&first_name=Shirish&last_name=Makwana&email=shirishm.makwana@gmail.com&password=shirish&pin_number=1234&mobile=9924400799
*
*/
...
if(!isset($_REQUEST["ws"]) || !isset($_REQUEST["first_name"]) || !isset($_REQUEST["last_name"]) || !isset($_REQUEST["email"]) || !isset($_REQUEST["password"]) || !isset($_REQUEST["pin_number"]) || !isset($_REQUEST["mobile"]))
{
...
else
{
	...
	if($first_name == "" || $email == "" || $password == "" || $pin_number == "" || $mobile == "")
	{
	...
	else
	{
		...
		else
		{
			...
			$sql = "INSERT INTO `users`(`first_name`, `last_name`, `email`, `password`, `pin_number`, `mobile`, `reg_datetime`, `verif_account`, `verif_code`, `verif_code_exp_datetime`, `verif_datetime`, `status`) VALUES ('$first_name' , '$last_name', '$email', '$password', '$pin_number', '$mobile', '$reg_datetime', '0', '$verif_code', '$exp_time', NULL, '$status')";
			...
			{				
				...
				$subject 	= "WOPADO - Verify your email";
				...
				$message 	= '
					...
					<head>
					<meta charset="utf-8">
					<title>Verify Account | Wopado</title>
					</head>
					...
					<table dir="ltr">
						<tbody>
							<tr>
								<td style="padding:0;font-family:\'Segoe UI Semibold\',\'Segoe UI Bold\',\'Segoe UI\',\'Helvetica Neue Medium\',Arial,sans-serif;font-size:17px;color:#551570">WOPADO account</td>
							</tr>
							<tr>
							...
							<tr>
								<td style="padding:0;padding-top:25px;font-family:\'Segoe UI\',Tahoma,Verdana,Arial,sans-serif;font-size:14px;color:#2a2a2a">Please use this code to verify your email at Wopado account <a dir="ltr" style="color:#2672ec;text-decoration:none" href="mailto:'.$email.'" target="_blank">'.$email.'</a>.</td>
							</tr>
							<tr>
