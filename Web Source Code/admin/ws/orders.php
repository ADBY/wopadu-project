...
/*
*	My Orders
* 	URL: http://localhost:8080/wopadu/ws/orders.php?ws=1&user_id=1&page=0
*/
...
if(!isset($_REQUEST['user_id']) || !isset($_REQUEST['page']))
{
...
else
{
	...
	$result = mysqli_query($link, "SELECT O.*, S.store_name, S.image as store_image FROM orders as O INNER JOIN stores as S ON O.store_id = S.id WHERE O.user_id = '$user_id' and O.payment_status = 1 order by O.id DESC LIMIT $offset, $limit");
