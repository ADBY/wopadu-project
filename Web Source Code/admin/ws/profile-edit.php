...
* 	URL: http://localhost:8080/wopadu/ws/profile-edit.php?user_id=1&first_name=Shirish&last_name=Makwana&mobile=9924400799&allergies=no alergies
*/
...
if(!isset($_REQUEST["user_id"]) || !isset($_REQUEST["first_name"]) || !isset($_REQUEST["last_name"]) || !isset($_REQUEST["mobile"]) || !isset($_REQUEST["allergies"]))
{
...
else
{
	...
	else
	{
		...
		else
		{
			...
			{
				$sql = "UPDATE `users` SET `first_name` = '$first_name', `last_name` = '$last_name', `mobile` = '$mobile', `allergies` = '$allergies' WHERE id = '$user_id'";
				...
				if( mysqli_query($link, $sql))
				{
					...
					print_r(json_encode($fetch, JSON_UNESCAPED_UNICODE));	// Categories array
				}
				else
				{
