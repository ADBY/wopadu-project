...
* 	URL: http://localhost:8080/wopadu/ws/store-categories.php?ws=1&store_id=1&lang=hi
*/
...
else
{
	...
	else
	{
		$lang_sql = "";
		if(isset($language)) {
			$lang_sql = ", `category_name_".$language['language_lc']."`";
		}
		$result = mysqli_query($link, "SELECT `id`, `store_id`, `parent_id`, `category_name`, `images`, `added_datetime`".$lang_sql." from categories WHERE store_id = '$store_id' and status = 1");
			...
			else
			{
				...
				while($row = mysqli_fetch_assoc($result))
				{
					if(isset($language))
					{
						...
						$category_name_lang 	= $row['category_name_'.$language['language_lc']];
						
						if($category_name_lang != "")
						{
							$row['category_name'] = $category_name_lang;
						}
						
						unset($row['category_name_'.$language['language_lc']]);						
					}
				...
				print_r(json_encode($categories, JSON_UNESCAPED_UNICODE));	// Categories array
			}
		}
	}
}
