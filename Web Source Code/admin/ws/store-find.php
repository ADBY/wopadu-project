	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				$q = "SELECT S.id, S.account_id, S.store_name, S.store_branch, S.address, S.description, S.tax_invoice, S.abn_number, S.image, S.welcome_notif, S.received_notif, S.ready_notif, S.display_note, S.secret_key, S.publishable_key, S.map_latitude, S.map_longitude, S.stripe_publishable_key, S.stripe_user_id, S.refresh_token, S.access_token, S.stripe_discount_percentage, S.status, A.account_name".$lang_sql." from stores as S INNER JOIN accounts as A ON A.id = S.account_id WHERE S.id = '$store_id' and S.status = 1";
