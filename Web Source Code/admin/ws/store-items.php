...
* 	URL: http://localhost:8080/wopadu/ws/store-items.php?ws=1&category_id=7&lang=hi
*/
...
else
{
	...
	else
	{
		$lang_sql = "";
		if(isset($language)) {
			$lang_sql = ", `item_name_".$language['language_lc']."`, `item_description_".$language['language_lc']."`";
		}
		
		$result = mysqli_query($link, "SELECT `id`, `category_id`, `kitchen_id`, `item_name`, `item_description`, `item_size`, `price`, `tax_percentage`, `images`, `has_variety`, `no_of_option`".$lang_sql." from items WHERE category_id = '$category_id' and status = 1");
			...
			else
			{
				...
				while($row = mysqli_fetch_assoc($result))
				{
					...
					if(isset($language))
					{
						...
						$item_name_lang 		= $row['item_name_'.$language['language_lc']];
						$item_description_lang 	= $row['item_description_'.$language['language_lc']];
						
						if($item_name_lang != "")
						{
							$row['item_name'] = $item_name_lang;
						}
						if($item_description_lang != "")
						{
							$row['item_description'] = $item_description_lang;
						}
						unset($row['item_name_'.$language['language_lc']]);
						unset($row['item_description_'.$language['language_lc']]);
					}
					...
					{	
						...
						if(mysqli_num_rows($res_option) > 0)
						{
							...
							while($row_option = mysqli_fetch_assoc($res_option))
							{
								$lang_sql = "";
								if(isset($language)) {
									$lang_sql = ", IOM.option_name_".$language['language_lc'].", IOS.sub_name_".$language['language_lc'];
								}
								
								$res_main_sub_option = mysqli_query($link, "SELECT IOM.id as item_option_main_id, IOS.id as item_option_sub_id, IOM.option_name, IOS.sub_name, IOS.sub_amount".$lang_sql." FROM item_option_main as IOM INNER JOIN item_option_sub as IOS ON IOS.option_id = IOM.id WHERE IOM.id = ".$row_option['option_main_id']);
								...
								if(mysqli_num_rows($res_main_sub_option) > 0)
								{
									while($row_main_sub_option = mysqli_fetch_assoc($res_main_sub_option))
									{
										if(isset($language))
										{
											$option_name 		= $row_main_sub_option['option_name'];
											$sub_name 			= $row_main_sub_option['sub_name'];
											$option_name_lang 	= $row_main_sub_option['option_name_'.$language['language_lc']];
											$sub_name_lang 		= $row_main_sub_option['sub_name_'.$language['language_lc']];
											
											if($option_name_lang != "")
											{
												$row_main_sub_option['option_name'] = $option_name_lang;
											}
											if($sub_name_lang != "")
											{
												$row_main_sub_option['sub_name'] = $sub_name_lang;
											}
											unset($row_main_sub_option['option_name_'.$language['language_lc']]);
											unset($row_main_sub_option['sub_name_'.$language['language_lc']]);
										}
									...
									if(isset($language)) {
										$sub[] = 
											[
												'item_option_sub_id' => $n['item_option_sub_id'],
												'sub_name' => $n['sub_name'],
												//'sub_name_'.$language['language_lc'] => $n['sub_name_'.$language['language_lc']],
												'sub_amount' => $n['sub_amount'],
											];
									} else {
										$sub[] = 
								...
								if(isset($language)) {
									$new[] = 
										[
											'item_option_main_id' => $m[0]['item_option_main_id'],
											'item_option_main_name' => $m[0]['option_name'],
											//'item_option_main_name_'.$language['language_lc'] => $n['option_name_'.$language['language_lc']],
											'options' => $sub,
										];
								} else {
									$new[] = 
					...
					{	
						...
						$res_variety = mysqli_query($link, "SELECT id, variety_name, variety_price".$lang_sql." FROM item_variety WHERE item_id = '$item_id'");
						...
						if(mysqli_num_rows($res_variety) > 0)
						{
							...
							while($row_variety = mysqli_fetch_assoc($res_variety))
							{
								if(isset($language))
								{
									$variety_name 		= $row_variety['variety_name'];
									$variety_name_lang 	= $row_variety['variety_name_'.$language['language_lc']];
									
									if($variety_name_lang != "")
									{
										$row_variety['variety_name'] = $variety_name_lang;
									}
									
									unset($row_variety['variety_name_'.$language['language_lc']]);						
								}
				...
				print_r(json_encode($items, JSON_UNESCAPED_UNICODE));	//	Items list array with options
			}
		}
	}
}
