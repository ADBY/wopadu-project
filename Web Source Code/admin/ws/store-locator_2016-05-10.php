...
else
{
	...
	$sql = "SELECT id, store_name, store_branch, address, image, description, map_latitude, map_longitude, ( 3959 * acos( cos( radians(".$map_latitude.") ) * cos( radians( map_latitude ) ) * cos( radians( map_longitude ) - radians(".$map_longitude.") ) + sin( radians(".$map_latitude.") ) * sin( radians( map_latitude ) ) ) ) AS distance".$lang_sql." FROM stores HAVING distance < $radius ORDER BY distance LIMIT 0 , 20";
