...
/*
*	Update stripe id
*	URL: http://localhost:8080/wopadu/ws/stripe-update.php?user_id=1&store_id=1&stripe_acc_id=213465fgsfkhsgdhg&cvv_number=235&card_number=4561346789&exp_date_month=03&exp_date_year=2017&secret_key=231456464&publishable_key=98754654&pin_number=4513
*	Type: 
*	1. iOs
*	2. Android
*/
			...
			else
			{
				...
				else
				{
					...
					else
					{
						...
						{
							 print_r('6');     // Stripe id does not exists for this store
							 exit;			
						}
						else
						{
							$row_strp = mysqli_fetch_assoc($check_strp);
							$strp_id = $row_strp['id'];
							
							if($card_number_db != $card_number || $cvv_number_db != $cvv_number || $expiry_date_month_db != $exp_date_month || $expiry_date_year_db != $exp_date_year)
							{
								$up_card = mysqli_query($link, "UPDATE users SET card_number = '$card_number', cvv_number = '$cvv_number', expiry_date_month = '$exp_date_month', expiry_date_year = '$exp_date_year' WHERE id = '$user_id'");
							...
							$sql = "UPDATE `stripe_accounts` SET `stripe_account_id`='$stripe_acc_id',`card_number`='$card_number',`cvv_number`='$cvv_number',`expiry_date_month`='$exp_date_month',`expiry_date_year`='$exp_date_year',`secret_key`='$secret_key',`publishable_key`='$publishable_key',`up_date`='$up_date' WHERE id = $strp_id";
							...
							{
								$data = mysqli_query($link,"SELECT * FROM stripe_accounts WHERE id=".$strp_id."") ;
