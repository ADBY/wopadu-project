...
local VariableTable = { background, header, restaurantName, restaurantAdd, homeBtn, searchBtn, menuScrollView,
		noCategory, CategoryTitleBg, CategoryTitle, backBg, backBtn, Language }
...
local function createNavigation()
   ...
   if(_HotelAddress:len() > 60) then
    ...
    NavigationVariableTable_pList.ProfileEditLabel = display.newText(tostring(GBCLanguageCabinet.getText("editProfileLabel",_LanguageKey)),NavigationVariableTable_pList.sliderBg.x+ _W/1.28 - _W/108,_H/3.25  + _H/18.11,_FontArr[6],_H/45 )
...
local function onStartDisplayFunc()
...
local k = 1
        ...
        if( #ProductIDArr > 0 ) then
        	...
        	noItem = display.newText( tostring(GBCLanguageCabinet.getText("NoItemsLabel",_LanguageKey)), _W/2,_H/2,_FontArr[26],_H/35 )
        	noItem:setTextColor( 57/255 ,9/255 ,78/255  )
        	displayGroup:insert(noItem)
        	noItem:toFront()
...
function scene:show( event )
    ...
    if ( phase == "will" ) then
		...
		VariableTable.placeOrderBtn = widget.newButton
		{
    		...
    		defaultFile = "images/cartBtn2.png",
   			...
   			label = tostring(GBCLanguageCabinet.getText("cartLabel",_LanguageKey)),
   			labelXOffset = 20,
    ...
    elseif ( phase == "did" ) then
        ...
        timer.performWithDelay( 250, displayFunc,1 )
