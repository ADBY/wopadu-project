...
local title, termsScrollView, heading
local termsGroup,termsRequest
...
local function termsListNetworkListener( event )
	...
	if ( event.isError ) then
		...
		if( networkReqCount > 3 ) then
			...
			termsRequest = network.request( url2, "GET", termsListNetworkListener )
		else
			local url = _WebLink.."site-terms.php?lang=".._LanguageKey
			local url2 = url:gsub( " ", "%%20" )
			termsRequest = network.request( url2, "GET", termsListNetworkListener )
		end
        ...
        if termsList == 0 then 
    		...
    		title.anchorX = 0
   			title.anchorY = 0
   			title.text = termsList.value
    		title:setFillColor( 83/255, 80/255, 79/255 )
    		termsScrollView:insert( title )
    		
    		termsScrollView:setScrollHeight( title.height + _H/4 )
...
function scene:show( event )
    ...
    if ( phase == "will" ) then
        ...
        termsGroup = display.newGroup()
        sceneGroup:insert(termsGroup)
        
        networkReqCount = 0
        
        termsScrollView = widget.newScrollView
		{
    		top = _H/12,
    		left = 0,
    		width = _W,
		...
		termsGroup:insert( termsScrollView )
...
function scene:hide( event )
    ...
    if ( phase == "will" ) then
        ...
        display.remove( termsScrollView )
        termsScrollView = nil
    ...
    elseif ( phase == "did" ) then
        ...
        if(termsRequest) then
        	network.cancel( termsRequest )
        	termsRequest = nil
        end
