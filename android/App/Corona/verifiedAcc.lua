...
local function handleOk3( event )
	...
	local body = "ws=1&user_id=".._UserID.."&send_email=1"
	local params = {}
	params.headers = headers
	params.body = body
	...
	if _UserID == nil then
		url = _WebLink.."account-verify.php?ws=1&user_id="..param.userIdForVerification.."&send_email=1"
	else
		url = _WebLink.."account-verify.php?ws=1&user_id=".._UserID.."&send_email=1"
	end
	local url2 = url:gsub( " ", "%%20" )
	verificationRequest1 = network.request( url2, "GET", verificationListNetworkListener2 )
...
local function verificationListNetworkListener( event )
	...
	if ( event.isError ) then
		...
		if( networkReqCount3 > 3 ) then  
			...
			local url
			if _UserID == nil then
				url = _WebLink.."account-verify.php?ws=1&user_id="..param.userIdForVerification.."&verif_code="..verificationCodeTfValue
			else
				url = _WebLink.."account-verify.php?ws=1&user_id=".._UserID.."&verif_code="..verificationCodeTfValue
			end
			local url2 = url:gsub( " ", "%%20" )
			verificationRequest2 = network.request( url2, "GET", verificationListNetworkListener )
			native.setActivityIndicator( true )
		end
    else
        print ( "RESPONSE:" .. event.response )
...
local function handleButtonEvent( event )
	if event.phase == "ended" then
		...
		if event.target.id == "verify" then
			...
			--param.userIdForVerification
			local body = "ws=1&user_id=".._UserID.."&verif_code="..verificationCodeTfValue
			local params = {}
			params.headers = headers
			params.body = body
    ...
    if ( phase == "will" ) then
        ...
        function verificationFunc( )
			...
			if composer.getSceneName( "previous" ) ~= "signUp" then
				...
				local url2 = url:gsub( " ", "%%20" )
				verificationRequest3 = network.request( url2, "GET", verificationListNetworkListener2 )
