...
local function networkListenerSubToSub2Image( event )
    ...
    elseif ( event.phase == "ended" ) then
        		...
        		if(#ProductCategoryID > 0) then
        			...
        			for i = 1, #ProductCategoryID do
						...
						if( _LanguageKey == "en" ) then
							...
							local url2 = url:gsub( " ", "%%20" )
							productRequest[i] = network.request( url2, "GET", productListNetworkListener )
						else
							local url = _WebLink.."store-items.php?ws=1&category_id="..ProductCategoryID[i].."&lang=".._LanguageKey
							local url2 = url:gsub( " ", "%%20" )
							productRequest[i] = network.request( url2, "GET", productListNetworkListener )
		...
		if( _LanguageKey == "en" ) then
			local url = _WebLink.."store-categories.php?ws=1&store_id=".._StoreID
			local url2 = url:gsub( " ", "%%20" )
			menuRequest = network.request( url2, "GET", menuListNetworkListener )
		else
			local url = _WebLink.."store-categories.php?ws=1&store_id=".._StoreID.."&lang=".._LanguageKey
			local url2 = url:gsub( " ", "%%20" )
			menuRequest = network.request( url2, "GET", menuListNetworkListener )
		end
...
local function checkStripeAccount()
	...
	local url = _WebLink.."stripe-view.php?user_id=".._UserID.."&store_id=".._StoreID
	local url2 = url:gsub( " ", "%%20" )
	stripeDetailsRequest = network.request( url2, "GET", stripeDataNetworkListener )
...
function fetchDataFromBeacon()
	...
	if( _LanguageKey == "en" ) then
		...
		local url = _WebLink.."store-find.php?beacon_major=".._majorBea
		local url2 = url:gsub( " ", "%%20" )
		beaconRequest = network.request( url2, "GET", BeaconDataNetworkListener )
	else
		local url = _WebLink.."store-find.php?beacon_major=".._majorBea.."&lang=".._LanguageKey
		local url2 = url:gsub( " ", "%%20" )
		beaconRequest = network.request( url2, "GET", BeaconDataNetworkListener )
...
local function getStripeKeysFunc()
	...
	local url2 = url:gsub( " ", "%%20" )
	s_keyRequest = network.request( url2, "GET", stripeKeyNetworkListener )
