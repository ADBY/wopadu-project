...
local downArrow, upArrow, sawAllOptionFlag,upTrans1,upTrans2,downTrans1,downTrans2,displayImageTimer
...
local function handleAddToCartButtonEvent( event )
	if( event.phase == "ended" ) then
		...
		local alert = native.showAlert( "Order added", GBCLanguageCabinet.getText("9Alert",_LanguageKey), { GBCLanguageCabinet.getText("YesLabel",_LanguageKey), GBCLanguageCabinet.getText("NoLabel",_LanguageKey) }, onGotoCartPage )
...
local function createNavigation()
   ...
   -- _UserID = "krishnamaru123@gmail.com"
    ...
    p_NavigationVariableTable.UserEmail = display.newText(UserMailText,-_W/1.28 + _W/15.42,_H/2.57,_FontArr[6],_H/55.15 )
...
function onDisplayProductImage() 
	 print( "on display product images?????????????????????????????????" )
	 if( composer.getSceneName( "current" ) == "Product" ) then
	 
	 if( _productImageFlag == true ) then
...
--        subMenuGroup:insert(VariableTable.AddToCartBtn)
    	...
    	VariableTable.AddToCartBtn = display.newText( "", _W/2, _H - _H/96 - _H/32, 0, _H/16.13, _FontArr[6], _H/30 )
		...
		if( e.phase == "moved" ) then
        	...
        	if ( dx > 10 ) then
        		if(addOnsScrollView[e.target.id]) then
		...
		return true
         ...
         if(_isNotesVisible ==  "1") then
			...
			VariableTable.NoteTextBox.isEditable = false
			VariableTable.NoteTextBox.font = native.newFont( _FontArr[41],_H/45  )
    		...
    		VariableTable.NoteTextLabel = display.newText( GBCLanguageCabinet.getText("addNotesLabel",_LanguageKey),VariableTable.NoteTextBox.x - VariableTable.NoteTextBox.width/2 + _W/108, VariableTable.NoteTextBox.y - VariableTable.NoteTextBox.height/2 + _H/76.8 ,_FontArr[41],_H/45 )
...
--    							addOnsScrollView[m]:insert(backText)
				...
				--RectOnTop[m].anchorY = 0
				RectOnTop[m].id = m
				RectOnTop[m]:setFillColor( 1,1,1,0.01 )
				RectOnTop[m]:addEventListener("touch",onTouchRect)
				RectOnTop[m].isVisible = false
				VariableTable.productNewScrollView:insert(RectOnTop[m])
    ...
    elseif ( phase == "did" ) then
        ...
        displayImageTimer = timer.performWithDelay( 2000, onDisplayProductImage ,-1 )
