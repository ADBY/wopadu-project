	...
	local function displayData( )
		...
		for i = 1, #restaurantList do
			...
			if( i == 1 ) then
				...
				restaurantDescri[i] = display.newText(descriValue,_W/4,address[i].y + address[i].height + _H/384,_W - _W/3.5,0,_FontArr[10],_H/40)
...
local function checkLocation( event )
	...
	if event.count == 10 then
		...
		if latitudeText:len() and longitudeText:len() > 0 then
			...
			Runtime:removeEventListener( "location", locationHandler )
			local alert = native.showAlert( alertLabel, "Please check your GPS connection of your device. Please try again later !!", { GBCLanguageCabinet.getText("okLabel",_LanguageKey) }, handleOk  )
		end
